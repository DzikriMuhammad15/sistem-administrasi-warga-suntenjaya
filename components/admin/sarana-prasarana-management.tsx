...
export function SaranaPrasaranaManagement() {
  const [items, setItems] = useState<SaranaPrasarana[]>([])
  ...
  const loadItems = async () => {
    ...
    if (error) {
      ...
      setMessage({ type: "error", text: "Gagal memuat data sarana prasarana" })
    } else {
      setItems(data || [])
    }
    setLoading(false)
  ...
  const handleEdit = (item: SaranaPrasarana) => {
    setFormData({
  ...
  return (
    <div className="p-6 space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-bold text-gray-900">Manajemen Sarana & Prasarana</h2>
          <p className="text-gray-600">Kelola data jumlah sarana dan prasarana yang tersedia</p>
        </div>
      ...
      {(isCreating || editingId) && (
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              {editingId
                ? "Ubah data sarana & prasarana"
                : "Tambahkan data baru untuk sarana & prasarana"}
            </CardDescription>
          </CardHeader>
          <CardContent className="space-y-4">
            <div>
              ...
              <Input
                ...
                placeholder="Contoh: Sekolah, Masjid, Puskesmas"
              />
            </div>
            <div>
