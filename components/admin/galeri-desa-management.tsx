...
export function GaleriDesaManagement() {
  const [items, setItems] = useState<GaleriDesa[]>([])
  ...
  const loadItems = async () => {
    ...
    if (error) {
      ...
      setMessage({ type: "error", text: "Gagal memuat data galeri desa" })
    } else {
      setItems(data || [])
    }
    setLoading(false)
  ...
  const handleEdit = (item: GaleriDesa) => {
    setFormData({
      judul: item.judul,
  ...
  const resetForm = () => {
    setFormData({ judul: "", link_foto: "", tanggal: "", kategori: "", deskripsi: "" })
  }
  ...
  return (
    <div className="p-6 space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-bold text-gray-900">Manajemen Galeri Desa</h2>
          <p className="text-gray-600">Kelola foto dan dokumentasi kegiatan desa</p>
        </div>
      ...
      {(isCreating || editingId) && (
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              {editingId ? "Ubah data galeri desa" : "Tambahkan data baru untuk galeri desa"}
            </CardDescription>
          </CardHeader>
          ...
          <div className="space-y-2">
            ...
            <div>
              ...
              <Textarea
                ...
                placeholder="Tambahkan detail deskripsi foto"
              />
            </div>
