...
export function SaranaOlahragaManagement() {
  const [items, setItems] = useState<SaranaOlahraga[]>([])
  ...
  const loadItems = async () => {
    ...
    if (error) {
      ...
      setMessage({ type: "error", text: "Gagal memuat data sarana olahraga" })
    } else {
      setItems(data || [])
    }
    setLoading(false)
  ...
  const handleEdit = (item: SaranaOlahraga) => {
    setFormData({
  ...
  return (
    <div className="p-6 space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-bold text-gray-900">Manajemen Sarana Olahraga</h2>
          <p className="text-gray-600">Kelola data jumlah sarana olahraga yang tersedia</p>
        </div>
      ...
      {(isCreating || editingId) && (
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              {editingId
                ? "Ubah data sarana olahraga"
                : "Tambahkan data baru untuk sarana olahraga"}
            </CardDescription>
          </CardHeader>
          <CardContent className="space-y-4">
            <div>
              ...
              <Input
                ...
                placeholder="Contoh: Lapangan Sepak Bola, GOR, Kolam Renang"
              />
            </div>
            <div>
